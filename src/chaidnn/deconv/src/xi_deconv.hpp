...
{
// Added pragmas to resolve issue of unknown memory size for function pointer parameters during HLS compilation
        #pragma HLS INTERFACE ap_bus port=in

	INTYPE_FLOAT tmp;					// local variable to read from DDR, then route into	BRAM
	#pragma HLS INTERFACE ap_bus port=tmp
...
#pragma HLS PIPELINE
				...
				// Not allowed to use memory copy in HLS synthesis except for bus interfaces, so it's replaced with a for loop equivalent in all original memcpy calls
...
{
	// Added pragmas to resolve issue of unknown memory size for function pointer parameters during HLS compilation
	#pragma HLS INTERFACE ap_bus port=gmemBias
...
{
	// Added pragmas to resolve issue of unknown memory size for function pointer parameters during HLS compilation
	#pragma HLS INTERFACE ap_bus port=deconvIDout
		...
		WBRAMTYPE biasArray[NMAX], OUTBRAMTYPE scoreBuffer[WOMAX], int idBuffer[WOMAX], int N, int Ri, int Ci, int Ro, int Co, int K, int DS, int logDS, int r)
{
	...
	#pragma HLS INTERFACE ap_bus port=tmpOutA
        #pragma HLS INTERFACE ap_bus port=tmpOutB
        #pragma HLS INTERFACE ap_bus port=tmpOutLocA
        #pragma HLS INTERFACE ap_bus port=tmpOutLocB
...
{
		// Added pragmas to resolve issue of unknown memory size for function pointer parameters during HLS compilation
		#pragma HLS INTERFACE ap_bus port=deconvIN
		#pragma HLS INTERFACE ap_bus port=deconvWT
		#pragma HLS INTERFACE ap_bus port=deconvBias
		#pragma HLS INTERFACE ap_bus port=deconvIDout
		#pragma HLS INTERFACE ap_bus port=scalar_deconv_args
