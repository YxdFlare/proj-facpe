...
// CHaiDNN paths
char* dirpath = "../../model/base1";
char* prototxt = "base1.prototxt";
char* caffemodel = "base1.caffemodel";
...
int main()
{
  ...
  while (i < ITEM_NBR && all_passing) {
    ...
    top(ENCODE,0,0,0,imgset_ptr,dcs_ptr,0);
    ...
    if ((MAX_LATENCY-1)%2)
      ...
      bound = MAX_LATENCY-1;
    for (int group_id = 0; group_id < bound; group_id++) {
      img_path1 = imgset_ptr + group_id * 2 * IMG_SIZE;
      img_path2 = img_path2 + IMG_SIZE;
    ...
    all_passing = !err_dft && !err_dut && !err_chai;       
